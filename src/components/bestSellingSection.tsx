...
export default function BestSelling() {
    const [time, setTime] = useState(1000 * 60 * 60 * 24 * 3 + 1000 * 60 * 60 * 23 + 1000 * 60 * 19 + 1000 * 56); // وقت مبدئي 3 أيام، 23 ساعة، 19 دقيقة، 56 ثانية
    ...
    const bestProducts = Products.filter((e:any)=> e.sales > 1500 && e.rate > 3)
        ...
        <section className="BestSelling my-20">
            <div className="container">
                ...
                <div className="titleAndViewAll flex mt-6 justify-between gap-5 max-sm:text-center w-full flex-wrap">
                    <h2 className="text-4xl font-semibold max-sm:text-2xl max-sm:mx-auto">Best Selling Products</h2>
                    <Link to='/bestSelling' className="bg-mainColor max-sm:mx-auto rounded-md text-white px-8 py-3">View All</Link>
                </div>
            </div>
            ...
            <div className="bestProducts4 flex mt-16 justify-center flex-wrap gap-8">
                {first4Products.map((product:any)=>(
                    <Card key={product.id} className="group mx-auto" sx={{ maxWidth: 320 }}>
                        ...
                        <CardContent>
                        <Typography gutterBottom variant="h6" sx={{height:'48px',paddingY:'10px',overflow: "hidden"}} component="h6">
                            {product.title}
                        </Typography>
            ...
            <div className="musicProduct px-12 max-md:gap-20 py-20 flex justify-between items-center font-semibold w-10/12 max-md:flex-col text-white bg-black mx-auto mt-32">
                <div className="left">
                    ...
                    <h2 className="text-5xl w-[380px] max-xl:w-auto max-lg:text-2xl max-xl:text-3xl max-md:text-center mt-8">Enhance Your Music Experience</h2>
                    <div className="bestProductCountDown flex gap-5 my-10 max-md:justify-center">
                        <div className="text-center size-16 max-lg:size-12 rounded-full bg-[#fff]">
                            <span className="block text-center max-lg:text-[16px] mt-1 text-black text-[20px] font-bold">{formatTime(Math.floor(time / (60 * 60 * 24 * 1000)))} <span className="text-mainColor"></span></span>
                            <span className="block text-[12px] max-lg:text-[9px] text-center text-gray-500">Days</span>
                        </div>
                        <div className="text-center size-16 max-lg:size-12 rounded-full bg-[#fff]">
                            <span className="block text-center max-lg:text-[16px] mt-1 text-black text-[20px] font-bold">{formatTime(Math.floor((time / (60 * 60 * 1000)) % 24))} <span className="text-mainColor"></span></span>
                            <span className="block text-[12px] max-lg:text-[9px] text-center text-gray-500">Hours</span>
                        </div>
                        <div className="text-center size-16 max-lg:size-12 rounded-full bg-[#fff]">
                            <span className="block text-center max-lg:text-[16px] mt-1 text-black text-[20px] font-bold">{formatTime(Math.floor((time / (60 * 1000)) % 60))} <span className="text-mainColor"></span></span>
                            <span className="block text-[12px] max-lg:text-[9px] text-center text-gray-500">Minutes</span>
                        </div>
                        <div className="text-center size-16 max-lg:size-12 rounded-full bg-[#fff]">
                            <span className="block text-center max-lg:text-[16px] mt-1 text-black text-[20px] font-bold">{formatTime(Math.floor((time / 1000) % 60))}</span>
                            <span className="block text-[12px] max-lg:text-[9px] text-center text-gray-500">Seconds</span>
                        </div>
                    </div>
                    <button className="bg-[#00FF66] block max-lg:text-sm max-md:mx-auto text-white max-lg:px-4 max-lg-py-2 px-8 py-3 rounded-md">Buy Now!</button>
                </div>
                <div className="right">
